...
export class DashboardFrontendStack extends Stack {
  constructor(
    scope: Construct,
    id: string,
    envName: string,
    ...
    const params: EnvParams = {
      ...
      zoneName: 'mycustomdomain.fi',
      publicCertRef: 'arn:aws:acm:us-east-1:ACCOUNTID:certificate/CERTID',
    };

    const domainNames = [`dashboard-${envName}.mycustomdomain.fi`];

    const dfBucket = new s3.Bucket(this, 'dashboard-frontend-' + envName, {
      bucketName: 'dashboard-frontend-' + envName,
    ...
    domainNames.forEach((recordName) => {
      const aRecord = new route53.ARecord(this, `DFAlias-${recordName}`, {
